...
def test_closed_bug_reactivation_rejects_requirement_pool_target_even_if_pool_looks_active(client: TestClient):
    ...
    bug = client.post(
        ...
        json={
            ...
            "title": "Pool target must be rejected",
    ...
    db = SessionLocal()
    try:
        active_state_id = db.query(Iteration.current_state_id).filter(Iteration.id == source_iteration_id).scalar()
    ...
    reactivated = client.post(
        ...
        json={"action_key": "activate", "payload": {"reason": "Pool target", "target_iteration_id": pool_id}},
        headers={"Authorization": f"Bearer {reporter_token}"},
    )

    assert reactivated.status_code == 409
    assert reactivated.json()["detail"]["code"] == "REQUIREMENT_POOL_OPERATION_FORBIDDEN"
