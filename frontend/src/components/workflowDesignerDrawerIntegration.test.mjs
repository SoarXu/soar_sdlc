...
assert.match(designer, /:object-type="activeObjectType"/)
...
assert.match(drawer, /objectType: \{ type: String, default: '' \}/)
assert.match(drawer, /label="系统状态角色"/)
assert.match(drawer, /v-if="stateRoleOptions\(props\.objectType\)\.length"/)
assert.match(drawer, /v-model="state\.state_role"/)
assert.match(drawer, /stateRoleOptions\(props\.objectType\)/)
assert.match(drawer, /emit\('move-transition'/)
assert.match(drawer, /emit\('add-transition'/)
assert.doesNotMatch(drawer, />\s*删除状态\s*</)
assert.doesNotMatch(drawer, /emit\('remove-state'/)
...
assert.match(drawer, /emit\('retarget-transition'/)
assert.match(drawer, /requestTransitionTargetChange/)
assert.match(drawer, /:disabled="!transitionEndpointsEnabled\(transition\)"/)
...
const setSelectedStateEnabledBody = functionBody(designer, 'setSelectedStateEnabled', 'removeSelectedTransition')
assert.match(setSelectedStateEnabledBody, /setWorkflowStateEnabled\(states\.value, transitions\.value, selectedState\.value\.id, enabled\)/)
assert.match(setSelectedStateEnabledBody, /states\.value = graph\.states/)
assert.match(setSelectedStateEnabledBody, /transitions\.value = graph\.transitions/)
assert.doesNotMatch(designer, /function removeSelectedState\(/)
