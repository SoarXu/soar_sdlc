    ...
    <div class="ldap-workbench">
      ...
      <section class="directory-panel">
        ...
        <template v-else>
          ...
          <div class="cursor-pagination"><span>共 {{ directoryTotal === null ? '总数未知' : directoryTotal }} 条</span><div class="cursor-pagination-actions"><el-button :icon="ArrowLeft" :disabled="directoryLoading || !cursorStack.length" @click="previousPage">上一页</el-button><span>第 {{ cursorStack.length + 1 }} / {{ directoryPageCount === null ? '?' : directoryPageCount }} 页</span><el-button :disabled="directoryLoading || !nextCursor" @click="nextPage">下一页<el-icon class="el-icon--right"><ArrowRight /></el-icon></el-button></div></div>
        </template>
      </section>
    </div>
...
const currentCursor = ref(null); const nextCursor = ref(null); const cursorStack = ref([]); const directoryTotal = ref(null)
...
const directoryPageCount = computed(() => directoryTotal.value === null ? null : Math.max(1, Math.ceil(directoryTotal.value / appliedDirectoryQuery.page_size)))
...
async function loadDirectory({ reset = false, cursor = reset ? null : currentCursor.value } = {}) { const requestId = ++directoryRequestId; directoryLoading.value = true; try { const { data } = await fetchLdapDirectoryUsers({ q: appliedKeyword.value || undefined, cursor: cursor || undefined, page_size: appliedDirectoryQuery.page_size, user_base_dn: appliedDirectoryQuery.user_base_dn, exclude_disabled: appliedDirectoryQuery.exclude_disabled }); if (requestId !== directoryRequestId) return false; directoryUsers.value = data.items || []; nextCursor.value = data.next_cursor || null; directoryTotal.value = data.total == null ? null : Number(data.total); if (reset) { currentCursor.value = null; cursorStack.value = [] } return true } catch (error) { if (requestId === directoryRequestId) ElMessage.error(actionErrorMessage(error)); return false } finally { if (requestId === directoryRequestId) directoryLoading.value = false } }
...
.cursor-pagination { justify-content: space-between; }.cursor-pagination-actions { display: flex; align-items: center; gap: 14px; }
</style>
