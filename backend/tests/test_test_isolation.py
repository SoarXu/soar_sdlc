...
def test_cleanup_created_rows_removes_project_and_requirement_pool_created_after_snapshot(client):
    before = _snapshot_table_ids()
    response = client.post("/api/v1/projects", json={"name": "cleanup-fixture-project"})
    assert response.status_code == 200
    project_id = response.json()["id"]
    requirement_pool_iteration_id = response.json()["requirement_pool_iteration_id"]
    ...
    try:
        ...
        pool_count = db.execute(
            text("select count(*) from iterations where id = :id"),
            {"id": requirement_pool_iteration_id},
        ).scalar_one()
        assert pool_count == 0
    finally:
        db.close()
