...
const requirementForm = reactive({ project_id: null, primary_component_id: null, iteration_id: null, title: '', requirement_type: '功能', priority: '3', owner_id: null, proposer: '', description: '', acceptance_criteria: '' })
...
const bugForm = reactive({ project_id: null, primary_component_id: null, iteration_id: null, requirement_id: null, task_id: null, test_case_id: null, test_run_id: null, title: '', bug_type: DEFAULT_BUG_TYPE_KEY, severity: '3', priority: '3', owner_id: null, proposer: '', reproduce_steps: '', expected_result: '', actual_result: '' })
...
function canManageIterationDelivery(row) { return canManageCurrentProject.value || row.owner_id === currentUser.value?.id }
...
function resetRequirementForm() { Object.assign(requirementForm, { project_id: projectId.value, primary_component_id: null, iteration_id: null, title: '', requirement_type: '功能', priority: '3', owner_id: null, proposer: '', description: '', acceptance_criteria: '' }) }
...
function resetBugForm() { Object.assign(bugForm, { project_id: projectId.value, primary_component_id: null, iteration_id: null, requirement_id: null, task_id: null, test_case_id: null, test_run_id: null, title: '', bug_type: DEFAULT_BUG_TYPE_KEY, severity: '3', priority: '3', owner_id: null, proposer: '', reproduce_steps: '', expected_result: '', actual_result: '' }) }
...
async function submitRequirement() {
  ...
  try {
    ...
    const payload = { ...formData, project_id: projectId.value, iteration_id: requirementForm.iteration_id, owner_id: requirementForm.owner_id || null, proposer: requirementForm.proposer || null }
...
async function submitTask() {
  ...
  saving.value = true
  try {
    const { primary_component_id, ...formData } = taskForm
    const payload = { ...formData, project_id: projectId.value, requirement_id: taskForm.requirement_id || null, task_type: deriveTaskBranch({ requirementId: taskForm.requirement_id, currentType: taskForm.task_type }), owner_id: taskForm.owner_id || null }
    if (!editingTaskId.value) payload.primary_component_id = primary_component_id || null
...
async function submitBug() {
  ...
  saving.value = true
  try {
    const { primary_component_id, ...formData } = bugForm
    const payload = { ...formData, project_id: projectId.value, iteration_id: bugForm.iteration_id || null, requirement_id: bugForm.requirement_id || null, task_id: bugForm.task_id || null, test_case_id: bugForm.test_case_id || null, test_run_id: bugForm.test_run_id || null, owner_id: bugForm.owner_id || null, proposer: bugForm.proposer || null }
    if (!editingBugId.value) payload.primary_component_id = primary_component_id || null
