...
@router.get("", response_model=list[WorkflowRuleRead], deprecated=True)
def get_workflow_rules(db: Session = Depends(get_db)):
    return list_workflow_rules(db)


@router.post("", response_model=WorkflowRuleRead, deprecated=True)
def post_workflow_rule(
...
@router.get("/components", response_model=list[WorkflowComponentRead], deprecated=True)
def get_workflow_components(db: Session = Depends(get_db)):
    return list_workflow_components(db)


@router.get("/templates", response_model=list[WorkflowTemplateRead], deprecated=True)
def get_workflow_templates():
    return list_workflow_templates()


@router.patch("/{rule_id}", response_model=WorkflowRuleRead, deprecated=True)
...
@router.delete("/{rule_id}", status_code=status.HTTP_204_NO_CONTENT, deprecated=True)
def remove_workflow_rule(
    rule_id: int,
    db: Session = Depends(get_db),
    _admin=Depends(require_system_admin),
):
