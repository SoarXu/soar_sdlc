...
CREATE TABLE IF NOT EXISTS project_members (
  ...
  user_id BIGINT UNSIGNED NOT NULL COMMENT '用户 ID',
  project_role VARCHAR(64) NOT NULL COMMENT '项目角色：product_owner、tech_lead、developer、test_lead、tester、viewer',
  is_default_assignee TINYINT(1) NOT NULL DEFAULT 0 COMMENT '是否默认分配人',
  is_workbench_participant TINYINT(1) NOT NULL DEFAULT 1 COMMENT '是否进入工作台范围',
...
CREATE TABLE IF NOT EXISTS requirements (
  ...
  owner_id BIGINT UNSIGNED NULL COMMENT '负责人 ID',
  proposer TEXT NULL COMMENT '提出人',
  ...
  CONSTRAINT fk_requirements_iteration FOREIGN KEY (iteration_id) REFERENCES iterations (id) ON DELETE RESTRICT
...
CREATE TABLE IF NOT EXISTS bugs (
  ...
  lifecycle_phase VARCHAR(32) NOT NULL DEFAULT 'development' COMMENT '生命周期阶段：development、maintenance',
  resolution VARCHAR(64) NULL COMMENT '解决结果',
  resolve_time DATETIME NULL COMMENT '解决时间',
  resolved_by BIGINT UNSIGNED NULL COMMENT '解决人',
  verify_result VARCHAR(64) NULL COMMENT '验证结果',
  verify_time DATETIME NULL COMMENT '验证时间',
  verified_by BIGINT UNSIGNED NULL COMMENT '验证人',
