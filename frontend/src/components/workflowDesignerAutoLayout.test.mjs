...
assert.match(applyTemplateBody, /applyGraph\(graph\.data\)/)
assert.doesNotMatch(applyTemplateBody, /layoutWorkflowWithElk\(/)
...
assert.match(source, /let saveGraphInFlight = null/)
assert.match(saveGraphBody, /if \(saveGraphInFlight\) return saveGraphInFlight/)
...
assert.match(confirmDiscardBody, /if \(saving\.value\) return false/)
