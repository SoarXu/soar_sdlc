    ...
    <div class="ldap-workbench">
      <section class="config-panel" v-loading="loading">
        ...
        <el-form ref="formRef" :model="form" :rules="rules" label-position="top" size="default">
          ...
          <div class="config-section"><h3>用户查询</h3><div class="form-grid">
            <el-form-item class="span-2" label="用户 Base DN"><el-input v-model="form.user_base_dn" placeholder="留空继承 Base DN" /></el-form-item>
            <el-form-item class="span-2" label="用户筛选器" prop="user_filter"><el-input v-model="form.user_filter" /></el-form-item>
            <el-form-item label="排除禁用账号"><el-switch v-model="form.exclude_disabled" /></el-form-item>
            <el-form-item label="每页数量"><el-input-number v-model="form.page_size" :min="10" :max="200" :step="10" controls-position="right" /></el-form-item>
          </div></div>
      ...
      <section class="directory-panel">
        ...
        <template v-else>
          ...
          <el-table ref="directoryTableRef" v-loading="directoryLoading" :data="directoryUsers" row-key="external_id" height="590" @selection-change="selectedRows = $event">
...
const rules = { host: [{ required: true, message: '请输入服务器地址', trigger: 'blur' }], port: [{ required: true, message: '请输入端口', trigger: 'change' }], base_dn: [{ required: true, message: '请输入 Base DN', trigger: 'blur' }], bind_username: [{ required: true, message: '请输入绑定账号', trigger: 'blur' }], user_filter: [{ required: true, message: '请输入用户筛选器', trigger: 'blur' }] }
...
function payload() { const value = { ...form }; delete value.has_bind_password; delete value.configuration_tested; delete value.tested_at; if (!value.bind_password) delete value.bind_password; value.user_base_dn = value.user_base_dn?.trim() || null; return value }
...
async function loadConfig() { loading.value = true; try { const data = (await fetchLdapConfig()).data; Object.assign(form, defaultForm(), data || {}, { bind_password: '' }); savedIdentity.value = data ? identityKey() : null; savedConfiguration.value = data ? configurationKey() : null; persistedEnabled.value = Boolean(data?.enabled) } catch (error) { ElMessage.error(actionErrorMessage(error)) } finally { loading.value = false } }
...
async function loadDirectory({ reset = false, cursor = reset ? null : currentCursor.value } = {}) { const requestId = ++directoryRequestId; directoryLoading.value = true; try { const { data } = await fetchLdapDirectoryUsers({ q: keyword.value.trim() || undefined, cursor: cursor || undefined, page_size: form.page_size }); if (requestId !== directoryRequestId) return false; directoryUsers.value = data.items || []; nextCursor.value = data.next_cursor || null; if (reset) { currentCursor.value = null; cursorStack.value = [] } return true } catch (error) { if (requestId === directoryRequestId) ElMessage.error(actionErrorMessage(error)); return false } finally { if (requestId === directoryRequestId) directoryLoading.value = false } }
function searchDirectory() { return loadDirectory({ reset: true }) }
