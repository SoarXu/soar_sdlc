...
1. 新增失败测试，模拟工号、姓名、邮箱、部门和唯一标识等属性返回空列表，期望映射结果为 `None`。
2. 运行目标测试，确认当前实现错误地产生字符串 `"[]"`。
3. 修改 LDAP 单值归一化函数，使空列表和空元组返回 `None`，非空集合仍取首项。
4. 运行全部 LDAP 测试，并通过真实目录 API 验证空工号不再显示为 `"[]"`。
5. 交付检查点：仅在主上明确确认后提交本任务文件。

### Task 12: 限定 LDAP 工作区滚动范围
...
1. 新增失败测试，要求桌面端页面和工作区占满主内容区剩余高度、左侧配置面板内部滚动、右侧表格使用弹性高度。
2. 运行 LDAP 页面测试，确认当前页面仍由左侧长内容撑开外层滚动。
3. 将 LDAP 页面设为两行网格，工作区使用 `minmax(0, 1fr)`；左侧配置面板设置 `overflow-y: auto`，右侧表格占用剩余空间。
4. 在 `900px` 以下恢复自动高度和可见溢出，使上下布局沿页面自然滚动。
5. 运行 LDAP 页面测试、前端全量测试和生产构建。
6. 交付检查点：仅在主上明确确认后提交本任务文件。
...
1. 新增失败测试，要求状态列宽度能够容纳 `82px` 标签和单元格左右内边距。
2. 运行 LDAP 页面测试，确认当前 `104px` 列宽不满足要求。
