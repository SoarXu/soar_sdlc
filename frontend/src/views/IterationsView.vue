      ...
      <el-table v-loading="loading" :data="pagedIterations" stripe>
        ...
        <el-table-column label="操作" width="210" fixed="right">
          <template #default="{ row }">
            <WorkflowActionButtons object-type="iteration" :object-id="row.id" mode="list" :transitions="workflowTransitions[row.id] || []" :auto-load="false" :users="users" @executed="loadData" />
...
async function loadData() {
  ...
  try {
    ...
    const { data } = await fetchWorkflowTransitionsBatch(iterations.value.map((item) => ({ object_type: 'iteration', id: item.id })))
    workflowTransitions.value = Object.fromEntries((data.items || []).map((item) => [item.id, item.transitions || []]))
  } catch {
...
async function submitIteration() {
  ...
  try {
    ...
    const payload = { ...formData, owner_id: form.owner_id || null }
