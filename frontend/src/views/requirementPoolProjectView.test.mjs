...
assert.match(projectDetail, /<el-table v-if="projectRequirementPoolRow" :data="projectRequirementPoolRow \? \[projectRequirementPoolRow\] : \[\]"/)
assert.match(projectDetail, /requirementIterationLabel\(row\)/)
assert.match(projectDetail, /v-if="canManageCurrentProject" link type="primary" @click="openIterationEdit\(row\)"/)
...
assert.match(projectDetail, /:disabled="editingRequirementPool"/)
assert.match(projectDetail, /const payload = editingRequirementPool\.value\n\s*\? \{ name: iterationForm\.name\.trim\(\) \}/)
assert.match(projectDetail, /<WorkflowActionButtons v-if="canManageCurrentProject && !row\.is_requirement_pool"/)
assert.match(projectDetail, /canManageCurrentProject && !row\.is_requirement_pool && iterationCanDefer\(row\)/)
